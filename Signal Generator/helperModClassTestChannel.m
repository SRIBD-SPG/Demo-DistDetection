  ...
  methods(Access = protected)
    function setupImpl(obj)
      obj.MultipathChannel = comm.RicianChannel(...
        ...
        'PathGainsOutputPort',obj.PathGainsOutputPort);
...
%     end
    ...
    function [signal,noise,H] = stepImpl(obj,x)
      % Add channel impairments
      
      [yInt1,H] = addMultipathFading(obj,x);
      yInt2 = addClockOffset(obj, yInt1);
      [signal,noise, H]  = addNoise(obj, yInt2, H);
    end

    function [out,H] = addMultipathFading(obj, in)
    ...
    function [out,Noise, H] = addNoise(obj,in, H)
      ...
      %   alpha=10^(unifrnd(-B,B)/10);
      Noise=sqrt(obj.ALPHA)*sqrt(1/2)*(randn((size(in,1)),1) + 1j*randn(size(in,1),1));
      Noise_power=sum(abs(Noise).^2)/length(Noise);
      Modulation_signal_power=sum(abs(in).^2)/length(in);
      ratio = sqrt(Noise_power*10^(obj.SNR/10)/Modulation_signal_power);
      sig = ratio*in;
      H = ratio*H;
      out = sig + Noise;
%       out = awgn(in,obj.SNR);
